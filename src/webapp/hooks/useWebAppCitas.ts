...
export function useWebAppCitas(pacienteId: number | undefined) {
  ...
  // pacienteNombre se mantiene por compatibilidad de firma; el nombre lo deriva el RPC server-side.
  const cancelarCita = useCallback(
    async (citaId: number, motivo: string, _pacienteNombre?: string) => {
      // 1. Cancelar vía RPC seguro (solo cancela citas del propio paciente)
      const { error: err } = await supabase.rpc('cancelar_cita_paciente', {
        p_cita_id: citaId,
        p_motivo: motivo,
      })
      if (err) {
      ...
      // 2. Notificar a médico + admins server-side y gateado (notificar_cancelacion deriva destinatarios
      //    + contenido; NO filtra el motivo al cuerpo/push). Reemplaza enviar-notificacion + enviar-push.
      //    Best-effort: no falla la cancelación.
      try {
