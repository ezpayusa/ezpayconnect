...
// Permisos data-driven de farmacia. La UI gatea leyendo los catálogos
...
export function useFarmaciaPermisos() {
  const { empresa, rol } = useProveedorAuth()
  const tipo = empresa?.tipo ?? null
  const [acciones, setAcciones] = useState<string[]>([])
  const [roles, setRoles] = useState<RolFarmacia[]>([])
