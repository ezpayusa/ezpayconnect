...
export default function LabPersonalPage() {
  const { empresa, cuenta } = useProveedorAuth()
  const { tienePermiso, roles, rolesAsignables, soyAdmin, loading: permLoading } = useLaboratorioPermisos()
  ...
  // === Activar/desactivar miembro vía cambiar_estado_miembro_proveedor ===
  // El RPC SECURITY DEFINER valida admin/misma-empresa/no-uno-mismo. Aquí solo se invoca.
  ...
  // El laboratorio no usa sucursales → sucursal_id: null.
  const invitar = async () => {
    ...
    const { data, error } = await supabase.functions.invoke('invitar-staff-proveedor', {
      body: { email: invEmail.trim(), nombre_completo: invNombre.trim(), rol: invRol, telefono: null, sucursal_id: null },
    })
  ...
  if (!permLoading && !tienePermiso('usuarios_roles')) {
    return (
      <div className="p-8 text-center">
        ...
        <p className="text-muted-foreground mt-2">Solo el Administrador gestiona el personal y los roles.</p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="p-4">
          <p className="text-sm font-semibold mb-3 flex items-center gap-2"><Users className="w-4 h-4" /> Roles de laboratorio</p>
                ...
                <CardContent className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <div className="flex flex-col sm:items-end gap-1.5 shrink-0">
                    <div className="flex items-center gap-2">
                      ...
                      {soyAdmin && !esYo && (
                        <Button variant="outline" size="sm" disabled={guardando === m.id}
      ...
      <Card>
        <CardContent className="p-4 space-y-3">
          ...
          <Button onClick={invitar} disabled={invitando} className="bg-[#0E7C6B] hover:bg-[#0b6357]">
