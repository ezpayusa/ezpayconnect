...
import { Loader2, UserPlus, AlertCircle, CheckCircle, Stethoscope } from 'lucide-react'
...
export default function RegistroMedicoPage() {
  ...
  useEffect(() => {
    ...
    const validar = async () => {
      try {
        const { data, error } = await supabase.functions.invoke('validar-invitacion-medico', {
          body: { token },
        })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('registrar-medico-invitacion', {
        body: {
          token,
          password,
        ...
        <Card className="w-full max-w-md">
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-gray-600 mb-4">
              Bienvenido, <strong>{invitacion?.nombre_completo}</strong>. Tu cuenta ha sido creada exitosamente.
            </p>
            <Link to="/login">
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <p className="text-sm text-gray-500">
            Invitación para: <strong>{invitacion?.nombre_completo}</strong>
          </p>
