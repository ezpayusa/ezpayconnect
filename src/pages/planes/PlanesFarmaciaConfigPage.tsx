...
import { ArrowLeft, Plus, Search, RefreshCw, Edit, Trash2, Store, Save, X, AlertTriangle } from 'lucide-react';
...
export default function PlanesFarmaciaConfigPage() {
  ...
  if (adminLoading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#e11d48]" />
      </div>
    );
  }
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <RefreshCw className="h-8 w-8 animate-spin text-[#e11d48]" />
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-7xl">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Gestiona planes para farmacias y droguerias</p>
          </div>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={recargar}><RefreshCw className="h-4 w-4 mr-2" /> Recargar</Button>
          <Button onClick={() => setDialogoCrear(true)} className="bg-[#e11d48] hover:bg-[#e11d48]/90"><Plus className="h-4 w-4 mr-2" /> Nuevo Plan</Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <Card>
          <CardContent className="pt-4">
            ...
            <p className="text-2xl font-bold">{planesFarmacia.length}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4">
            <p className="text-sm text-muted-foreground">Configuraciones</p>
            <p className="text-2xl font-bold">{configsFarmacia.length}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4">
        ...
        <Card>
          <CardContent className="pt-4">
            ...
            <p className="text-2xl font-bold">{planesFarmacia.length > 0 ? Math.max(...planesFarmacia.map(p => p.precio_base)) : 0} USD</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="mb-6">
        <CardHeader>
          <CardTitle className="text-lg">Planes Base Farmacia</CardTitle>
        </CardHeader>
        <CardContent>
                ...
                {planesFiltrados.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                      No se encontraron planes farmacia
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
            ...
            <Table>
              ...
              <TableBody>
                ...
                {configsFarmacia.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                      No hay configuraciones de farmacia
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={handleCrearPlan} disabled={!nuevoPlan.nombre} className="bg-[#e11d48] hover:bg-[#e11d48]/90">
      ...
      <Dialog open={!!dialogoEditar} onOpenChange={() => setDialogoEditar(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Edit className="h-5 w-5 text-[#e11d48]" />
              Editar Plan Farmacia
            </DialogTitle>
          </DialogHeader>
          {dialogoEditar && (
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button onClick={handleEditarPlan} className="bg-[#e11d48] hover:bg-[#e11d48]/90">
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={handleCrearConfig} disabled={!nuevaConfig.plan_base_id || !nuevaConfig.pais_id} className="bg-[#e11d48] hover:bg-[#e11d48]/90">
