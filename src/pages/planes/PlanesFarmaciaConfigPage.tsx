...
function DraggableWindow({ 
  ...
  return (
    <>
      ...
      >
        <div 
          className="drag-handle flex items-center justify-between p-4 border-b cursor-move bg-rose-50 rounded-t-lg select-none"
...
export default function PlanesFarmaciaConfigPage() {
  ...
  const configsFarmacia = planesConfig.filter(p => p.plan_base?.tipo === 'farmacia');

  const planesFiltrados = planesFarmacia.filter(p => 
    ...
    <div className="container mx-auto px-4 py-6 max-w-7xl">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">Configuracion Planes Farmacia</h1>
            <p className="text-sm text-muted-foreground">Gestiona planes para farmacias y dispensarios de medicamentos</p>
          </div>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={recargar}><RefreshCw className="h-4 w-4 mr-2" /> Recargar</Button>
          <Button onClick={() => setDialogoCrear(true)} className="bg-rose-500 hover:bg-rose-600"><Plus className="h-4 w-4 mr-2" /> Nuevo Plan</Button>
        </div>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <Card><CardContent className="pt-4"><p className="text-sm text-muted-foreground">Planes Base</p><p className="text-2xl font-bold">{planesFarmacia.length}</p></CardContent></Card>
        <Card><CardContent className="pt-4"><p className="text-sm text-muted-foreground">Configuraciones</p><p className="text-2xl font-bold">{configsFarmacia.length}</p></CardContent></Card>
        ...
        <Card><CardContent className="pt-4"><p className="text-sm text-muted-foreground">Precio Base Max</p><p className="text-2xl font-bold">{planesFarmacia.length > 0 ? Math.max(...planesFarmacia.map(p => p.precio_base)) : 0} USD</p></CardContent></Card>
      </div>

      {/* Tabla de Planes Base */}
      <Card className="mb-6">
        <CardHeader><CardTitle className="text-lg">Planes Base Farmacia</CardTitle></CardHeader>
        <CardContent>
            ...
            <Table>
              ...
              <TableBody>
                ...
                {planesFiltrados.length === 0 && (
                  <TableRow><TableCell colSpan={9} className="text-center py-8 text-muted-foreground">No se encontraron planes farmacia</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </div>
          ...
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                ...
                {configsFarmacia.length === 0 && (
                  <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">No hay configuraciones de farmacia</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </div>
      ...
      {/* ════════════════════════════════════════════════════
          VENTANA ARRASTRABLE: Nuevo Plan Farmacia
          ════════════════════════════════════════════════════ */}
      <DraggableWindow
        isOpen={dialogoCrear}
        onClose={() => setDialogoCrear(false)}
        title="Nuevo Plan Farmacia"
        initialPosition={{ x: typeof window !== 'undefined' ? window.innerWidth / 2 - 250 : 100, y: 100 }}
      >
        <div className="space-y-4">
          <div>
            <Label htmlFor="nombre">Nombre *</Label>
            <Input id="nombre" value={nuevoPlan.nombre} onChange={(e) => setNuevoPlan({...nuevoPlan, nombre: e.target.value})} placeholder="Ej: Farmacia Premium" />
          </div>
          <div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button onClick={handleCrearPlan} disabled={!nuevoPlan.nombre} className="bg-rose-500 hover:bg-rose-600">
      ...
      {/* ════════════════════════════════════════════════════
          VENTANA ARRASTRABLE: Editar Plan Farmacia
          ════════════════════════════════════════════════════ */}
      <DraggableWindow
        isOpen={!!dialogoEditar}
        onClose={() => setDialogoEditar(null)}
        title="Editar Plan Farmacia"
        initialPosition={{ x: typeof window !== 'undefined' ? window.innerWidth / 2 - 250 : 100, y: 100 }}
      >
        {dialogoEditar && (
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button onClick={handleEditarPlan} className="bg-rose-500 hover:bg-rose-600">
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button onClick={handleCrearConfig} disabled={!nuevaConfig.plan_base_id || !nuevaConfig.pais_id} className="bg-rose-500 hover:bg-rose-600">
