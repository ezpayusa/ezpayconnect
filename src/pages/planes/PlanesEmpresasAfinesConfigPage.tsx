...
function DraggableWindow({ 
  ...
  return (
    <>
      ...
      >
        <div 
          className="drag-handle flex items-center justify-between p-4 border-b cursor-move bg-indigo-50 rounded-t-lg select-none"
    ...
    <div className="container mx-auto px-4 py-6 max-w-7xl">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">Configuracion Planes Empresas Afines</h1>
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => setDialogoCrear(true)} className="bg-indigo-500 hover:bg-indigo-600"><Plus className="h-4 w-4 mr-2" /> Nuevo Plan</Button>
        </div>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <Card><CardContent className="pt-4"><p className="text-sm text-muted-foreground">Planes Base</p><p className="text-2xl font-bold">{planesEmpresasAfines.length}</p></CardContent></Card>
        <Card><CardContent className="pt-4"><p className="text-sm text-muted-foreground">Configuraciones</p><p className="text-2xl font-bold">{configsEmpresasAfines.length}</p></CardContent></Card>
        ...
        <Card><CardContent className="pt-4"><p className="text-sm text-muted-foreground">Precio Base Max</p><p className="text-2xl font-bold">{planesEmpresasAfines.length > 0 ? Math.max(...planesEmpresasAfines.map(p => p.precio_base)) : 0} USD</p></CardContent></Card>
      </div>

      {/* Tabla de Planes Base */}
      <Card className="mb-6">
        <CardHeader><CardTitle className="text-lg">Planes Base Empresas Afines</CardTitle></CardHeader>
        <CardContent>
            ...
            <Table>
              ...
              <TableBody>
                ...
                {planesFiltrados.length === 0 && (
                  <TableRow><TableCell colSpan={9} className="text-center py-8 text-muted-foreground">No se encontraron planes empresas_afines</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </div>
            ...
            <Table>
              ...
              <TableBody>
                ...
                {configsEmpresasAfines.length === 0 && (
                  <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">No hay configuraciones de empresas_afines</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </div>
      ...
      {/* ════════════════════════════════════════════════════
          VENTANA ARRASTRABLE: Nuevo Plan Empresas Afines
          ════════════════════════════════════════════════════ */}
      <DraggableWindow
        isOpen={dialogoCrear}
        onClose={() => setDialogoCrear(false)}
        title="Nuevo Plan Empresas Afines"
        initialPosition={{ x: typeof window !== 'undefined' ? window.innerWidth / 2 - 250 : 100, y: 100 }}
      >
        <div className="space-y-4">
          <div>
            <Label htmlFor="nombre">Nombre *</Label>
            <Input id="nombre" value={nuevoPlan.nombre} onChange={(e) => setNuevoPlan({...nuevoPlan, nombre: e.target.value})} placeholder="Ej: Empresa Afiliada Premium" />
          </div>
          <div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button onClick={handleCrearPlan} disabled={!nuevoPlan.nombre} className="bg-indigo-500 hover:bg-indigo-600">
      ...
      {/* ════════════════════════════════════════════════════
          VENTANA ARRASTRABLE: Editar Plan Empresas Afines
          ════════════════════════════════════════════════════ */}
      <DraggableWindow
        isOpen={!!dialogoEditar}
        onClose={() => setDialogoEditar(null)}
        title="Editar Plan Empresas Afines"
        initialPosition={{ x: typeof window !== 'undefined' ? window.innerWidth / 2 - 250 : 100, y: 100 }}
      >
        {dialogoEditar && (
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button onClick={handleEditarPlan} className="bg-indigo-500 hover:bg-indigo-600">
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button onClick={handleCrearConfig} disabled={!nuevaConfig.plan_base_id || !nuevaConfig.pais_id} className="bg-indigo-500 hover:bg-indigo-600">
