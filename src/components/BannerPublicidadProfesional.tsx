...
// ============================================================
// Pub-C · Banner publicitario para superficies PROFESIONALES (médico / clínica).
// ------------------------------------------------------------
// Sección DEDICADA. NUNCA dentro del flujo ni de la query de prescripción
// (useBusquedaMedicamentos, farmacia_medicamentos, productos_empresa). No mezcla
// ads con el catálogo clínico. Lee campanas_publicitarias directo; el filtrado
// POR PAÍS lo hace la RLS de Pub-B (mi_pais_viewer()) en el servidor — aquí NO se
// añade lógica de país en el cliente (a diferencia del banner del consumidor, que
// pre-filtra con usePaisFiltro). Sin migración, sin cambio de RLS.
// ============================================================
...
interface Props {
  /** Para métricas: superficie donde se muestra el banner. */
  tipoPerfil: 'medico' | 'clinica'
  /** Para métricas: contexto fino (default = dashboard). */
  contexto?: string
}

export default function BannerPublicidadProfesional({ tipoPerfil, contexto = 'dashboard' }: Props) {
  ...
  useEffect(() => {
    const fetchCampanas = async () => {
      // País lo filtra la RLS (mi_pais_viewer). NO se añade .eq('pais_id', ...) aquí.
  ...
  useEffect(() => {
    if (campanaActual && !yaRegistradas.current.has(campanaActual.id)) {
      ...
      registrarMetricaCampana(campanaActual.id, { tipoPerfil, contexto })
    }
  }, [campanaActual, tipoPerfil, contexto])
      ...
      <div className="p-4">
        ...
        {campanaActual.link_url && (
          <Button
            ...
            className="mt-3"
            onClick={() => {
              registrarMetricaCampana(campanaActual.id, { tipoPerfil, contexto, clickeado: true })
