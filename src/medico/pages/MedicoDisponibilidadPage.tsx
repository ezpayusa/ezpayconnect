...
import { ArrowLeft, Plus, Trash2, Loader2, Clock, Stethoscope, Briefcase } from 'lucide-react'
import { Badge } from '@/components/ui/badge'

const DIAS = ['Domingo', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado']
type Contexto = 'visitador' | 'paciente'
...
export default function MedicoDisponibilidadPage() {
  ...
  const [tab, setTab] = useState<Contexto>('visitador')
  const [mostrarForm, setMostrarForm] = useState(false)
  const [form, setForm] = useState({ dia_semana: 1, hora_inicio: '09:00', hora_fin: '12:00', duracion_slot: 30, clinica_id: '' })

  const esVisitador = tab === 'visitador'

  const cambiarTab = (t: Contexto) => { setTab(t); setMostrarForm(false) }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const ok = await crearSlot({
      ...
      duracion_slot: esVisitador ? 15 : form.duracion_slot,
  ...
  const delTab = disponibilidad.filter((d) => ((d as any).contexto || 'visitador') === tab)
  const agrupados = delTab.reduce((acc: Record<string, typeof delTab>, item) => {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-[#1a2a3a] flex items-center gap-2">
            <Clock className="h-7 w-7 text-[#1E5C8E]" /> Mi Disponibilidad
          </h1>
          <p className="text-sm text-muted-foreground">
            Tienes dos agendas independientes: una para visitadores médicos y otra para pacientes.
          </p>
        </div>
      </div>
      ...
      <p className="text-sm text-muted-foreground">
        {esVisitador
          ? 'Horarios en los que atiendes a visitadores médicos. Cada cita dura 15 minutos (fijo).'
          : 'Horarios en los que atiendes a pacientes. Estos horarios los usa el portal del paciente para agendar.'}
      </p>

      <div className="flex justify-end">
        <Button onClick={() => setMostrarForm(!mostrarForm)}>
          <Plus className="h-4 w-4 mr-1" /> {mostrarForm ? 'Cancelar' : 'Agregar horario'}
        </Button>
      </div>
      ...
      {mostrarForm && (
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">Nuevo horario · {esVisitador ? 'Visitadores' : 'Pacientes'}</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              {esVisitador ? (
                ...
                <div className="space-y-2">
                  ...
                  <Input type="number" min={10} step={5} value={form.duracion_slot} onChange={(e) => setForm({ ...form, duracion_slot: parseInt(e.target.value) })} required />
                </div>
              )}
      ...
      {loading ? (
        ...
        <Card className="bg-gray-50 border-dashed">
          <CardContent className="p-8 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-700">Sin horarios para {esVisitador ? 'visitadores' : 'pacientes'}</h3>
            <p className="text-sm text-muted-foreground max-w-md mx-auto mt-1">
              {esVisitador
                ? 'Agrega horarios para que los visitadores médicos puedan agendar visitas contigo.'
                : 'Agrega horarios para que los pacientes puedan agendar citas desde el portal.'}
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-4">
          {Object.entries(agrupados).map(([dia, slots]) => (
            <Card key={dia}>
              <CardHeader className="pb-2"><CardTitle className="text-base font-semibold">{dia}</CardTitle></CardHeader>
              <CardContent className="space-y-2">
                {slots.map((slot) => (
                  <div key={slot.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div className="flex items-center gap-3">
                      <Clock className="h-4 w-4 text-[#1E5C8E]" />
                      <span className="text-sm font-medium">{slot.hora_inicio} – {slot.hora_fin}</span>
                      <Badge variant="outline" className="text-xs">{slot.duracion_slot} min/cita</Badge>
                      {slot.clinica_id && (
                    ...
                    <Button size="sm" variant="ghost" className="text-red-600 hover:bg-red-50" onClick={() => eliminarSlot(slot.id)}>
