...
- CERRADO #1: contexto_ia_paciente ahora incluye SOAP historico (subjetivo/objetivo/analisis trunc 500) — RPC (b2d6e99, supabase/fixes/contexto_ia_paciente_soap_hist_01.sql, -f) + edge asistente-ia render S/O/A + seccion "CONSULTAS PREVIAS (SOAP)" (a7f63aa, deploy con nombre). La IA ya ve el `analisis` del medico (antes ignorado). Smoke: consulta con historial en prod (edge CORS cerrado, no preview).
- PENDIENTE: #3 (linkage cita_id en recetas + consulta_id en examenes/vitales).
- ef42bde fix(db): FK faltante receta_items.receta_id -> recetas(id) ON DELETE CASCADE (supabase/fixes/receta_items_fk_recetas_01.sql). El embed PostgREST recetas(...,receta_items(...)) de la webapp del paciente fallaba con "Could not find a relationship" (columna existia sin constraint; el DDL de hoy forzo el reload de PostgREST y destapo la FK ausente). Huerfanos=0. Endurece el fix del N+1. Parte del #3: receta_items->recetas queda firme; sigue pendiente recetas.cita_id + consulta_id en examenes/vitales.
