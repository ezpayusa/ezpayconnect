-- ############################################################################################
-- Rollback de la migracion 329 - cuerpos previos VERBATIM (capturados 25-sep, sin CR)
-- ############################################################################################
-- OJO: reabre el despacho de recetas canceladas. Solo para volver atras la 329.
-- ############################################################################################

BEGIN;

-- verificar_receta_despacho(text): cuerpo previo verbatim (md5 a05b6cc3a033bf5dbdfa6403b1892686)
...
-- registrar_dispensacion(text,bigint[],text): cuerpo previo verbatim (md5 a5aeee75f871b27641f122fcaca9e29b)
...
-- registrar_dispensacion_dirigida(bigint,bigint[],text): cuerpo previo verbatim (md5 9dd60e8d853e80696707b03a325b8cb8)
...
-- revelar_items_receta(bigint,text): cuerpo previo verbatim (md5 5b4acb0927be18b854489619fceed730)
...
-- crear_entrega(bigint,integer): cuerpo previo verbatim (md5 e142da2e8ba0db1226f500be5056a4f9)
...
-- fijar_modalidad_grupo(bigint,integer,text): cuerpo previo verbatim (md5 a19f2ed5324ad07feee5bf07ae2d8825)
...
BEGIN
  FOR e IN SELECT * FROM (VALUES
    ('public.verificar_receta_despacho(text)', 'a05b6cc3a033bf5dbdfa6403b1892686', true, 'postgres', '{"search_path=\"\""}', '{authenticated=X/postgres,postgres=X/postgres,service_role=X/postgres}'),
    ('public.registrar_dispensacion(text,bigint[],text)', 'a5aeee75f871b27641f122fcaca9e29b', true, 'postgres', '{"search_path=\"\""}', '{authenticated=X/postgres,postgres=X/postgres,service_role=X/postgres}'),
    ('public.registrar_dispensacion_dirigida(bigint,bigint[],text)', '9dd60e8d853e80696707b03a325b8cb8', true, 'postgres', '{"search_path=\"\""}', '{authenticated=X/postgres,postgres=X/postgres,service_role=X/postgres}'),
    ('public.revelar_items_receta(bigint,text)', '5b4acb0927be18b854489619fceed730', true, 'postgres', '{"search_path=\"\""}', '{authenticated=X/postgres,postgres=X/postgres,service_role=X/postgres}'),
    ('public.crear_entrega(bigint,integer)', 'e142da2e8ba0db1226f500be5056a4f9', true, 'postgres', '{"search_path=\"\""}', '{authenticated=X/postgres,postgres=X/postgres,service_role=X/postgres}'),
    ('public.fijar_modalidad_grupo(bigint,integer,text)', 'a19f2ed5324ad07feee5bf07ae2d8825', true, 'postgres', '{"search_path=\"\""}', '{authenticated=X/postgres,postgres=X/postgres,service_role=X/postgres}')
    ...
    IF position('(mig 329)' in r.prosrc) > 0 THEN v := v||E'\n '||e.f||' conserva el chequeo de la 329'; END IF;
  END LOOP;
  ...
  IF v <> '' THEN RAISE EXCEPTION 'ROLLBACK329 AUTOCHEQUEO FALLA:%', v; END IF;
END $ac$;
