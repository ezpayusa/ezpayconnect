-- ############################################################################################
-- 331 ROLLBACK - vuelve capturar_signo_vital a la definicion EXACTA de la 330 (md5 b2246251fd9154ebcb851332991170c7),
-- capturada de prod el 25-sep con pg_get_functiondef, byte a byte.
-- NO reinserta la fila 1453: era una toma con los 9 vitales en NULL, basura del bug que la 331
-- corrige; no hay nada clinico que restaurar. Los 8 CHECK sv_*_rango de la 330 no se tocan.
-- ############################################################################################
...
BEGIN
  ...
  IF position('SV003' in r.src) > 0 THEN bad := bad||'quedo SV003; '; END IF;
  ...
  IF bad <> '' THEN RAISE EXCEPTION 'ROLLBACK331 AUTOCHEQUEO FALLA:%', bad; END IF;
END $chk$;
