-- ============================================================
-- (u) · emitir_receta rechaza medicamentos no activos.
-- Reemplaza la mig 242 (CREATE OR REPLACE). Todo lo demas es identico.
--
-- HALLAZGO (10 jul): public.medicamentos tiene 40 filas = 20 genericos
-- DUPLICADOS 1:1. ids 1-20 con activo=null (copia fantasma), ids 21-40 con
-- activo=true. `activo` es nullable SIN DEFAULT: un INSERT que no la setee
-- nace invisible. useMedicamentos filtra .eq('activo', true), asi que los
-- fantasmas nunca llegan al medico. La RPC NO lo validaba: un medicamento_id
-- fabricado a mano pasaba el gate PR005.
--
-- ASIMETRIA DELIBERADA de los NULL:
--   categoria_regulatoria NULL -> NO regulado  (permisivo: no bloquear una
--     receta legitima por falta de clasificacion)
--   activo NULL -> NO activo   (restrictivo: no recetar un farmaco que quiza
--     se retiro)
-- El criterio no es "NULL es falso". Es: ante la duda, la opcion que menos
-- daño hace.
--
-- items_con_med_inactivo = 0 al momento de aplicar: nada se rompe.
-- NO cierra el duplicado (s): eso es O2b (deduplicar ANTES de clasificar).
-- ============================================================
