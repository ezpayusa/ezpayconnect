...
serve(async (req) => {
  ...
  try {
    ...
    const { token, password, nombre_completo, telefono, especialidad } = body
    ...
    if (authError) {
      ...
      console.error('[registrar-medico-invitacion] Auth error:', authError)
      return new Response(
    ...
    // 3. Insertar en perfiles primero (la función SQL espera que exista)
    await supabase.from('perfiles').insert({
      id: userId,
      email: invitacion.email,
      nombre_completo: nombre_completo || invitacion.nombre_completo,
      rol: 'medico',
    ...
    const { data: result, error: rpcError } = await supabase.rpc('registrar_medico_desde_invitacion', {
      p_token: token,
      p_user_id: userId,
      p_email: invitacion.email,
      p_nombre_completo: nombre_completo || invitacion.nombre_completo,
      p_telefono: telefono || invitacion.telefono,
      p_especialidad: especialidad || invitacion.especialidad,
    })

    if (rpcError) {
      console.error('[registrar-medico-invitacion] RPC error:', rpcError)
      return new Response(
    ...
    console.error('[registrar-medico-invitacion] Error:', error)
    return new Response(
