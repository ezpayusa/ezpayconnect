...
DO $$ DECLARE n int; BEGIN
  ...
  ELSE
    SELECT count(*) INTO n FROM public.farmacias WHERE id IN (NULLIF(current_setting('probe.p0_fgt',true), '')::int, NULLIF(current_setting('probe.p0_fhn',true), '')::int, NULLIF(current_setting('probe.cat_fA',true), '')::int);
...
DO $$ DECLARE v_pac uuid; v_clin uuid; BEGIN
  ...
  SELECT auth_user_id INTO v_pac FROM public.pacientes WHERE auth_user_id IS NOT NULL AND pais_id = NULLIF(current_setting('probe.p0_gt',true), '')::uuid LIMIT 1;
  SELECT id INTO v_clin FROM public.perfiles WHERE rol='admin_clinica' AND pais_id = NULLIF(current_setting('probe.p0_gt',true), '')::uuid LIMIT 1;
  ...
  INSERT INTO public.campanas_publicitarias (titulo,tipo,fecha_inicio,fecha_fin,activa,peso,pais_id)
    VALUES ('PB GT','banner',CURRENT_DATE-1,CURRENT_DATE+30,true,1, NULLIF(current_setting('probe.p0_gt',true), '')::uuid);
  INSERT INTO public.campanas_publicitarias (titulo,tipo,fecha_inicio,fecha_fin,activa,peso,pais_id)
    VALUES ('PB HN','banner',CURRENT_DATE-1,CURRENT_DATE+30,true,1, NULLIF(current_setting('probe.p0_hn',true), '')::uuid);
  INSERT INTO public.campanas_publicitarias (titulo,tipo,fecha_inicio,fecha_fin,activa,peso,pais_id)
    VALUES ('PB GT FUT','banner',CURRENT_DATE+5,CURRENT_DATE+30,true,1, NULLIF(current_setting('probe.p0_gt',true), '')::uuid);
...
DO $$ DECLARE v_medhn uuid; BEGIN
  ...
  v_medhn := NULLIF(current_setting('probe.p0_mednull',true), '')::uuid;
  DELETE FROM public.planes_visitador_contratados WHERE empresa_id = NULLIF(current_setting('probe.pa_ea',true), '')::uuid;  -- estado limpio (P242 de Vis-A dejó un plan)
...
DO $$ DECLARE n int; BEGIN
  ...
  ELSE SELECT count(*) INTO n FROM public.disponibilidad_medico WHERE medico_id=NULLIF(current_setting('probe.fm_medgt',true), '')::uuid AND contexto='visitador';
...
DO $$ DECLARE n int; BEGIN
  ...
  ELSE SELECT count(*) INTO n FROM public.disponibilidad_medico WHERE medico_id=NULLIF(current_setting('probe.vg1_medhn',true), '')::uuid AND contexto='visitador';
...
DO $$ DECLARE n int; BEGIN
  ...
  ELSE SELECT count(*) INTO n FROM public.disponibilidad_medico WHERE medico_id=NULLIF(current_setting('probe.fm_medgt',true), '')::uuid AND contexto='paciente';
...
DO $$ BEGIN IF current_setting('probe.vg1_ready',true)='1' THEN
  UPDATE public.planes_visitador_contratados SET fecha_fin = CURRENT_DATE-1 WHERE empresa_id=NULLIF(current_setting('probe.pa_ea',true), '')::uuid AND pais_id=NULLIF(current_setting('probe.p0_gt',true), '')::uuid;
END IF; EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.b2_fallos', coalesce(current_setting('probe.b2_fallos', true),'')||'L4409('||SQLSTATE||') ', false); END $$;
SELECT set_config('request.jwt.claims', json_build_object('sub', current_setting('probe.cat_inv',true), 'role','authenticated')::text, true);
SELECT set_config('role','authenticated',true);
DO $$ DECLARE n int; BEGIN
...
DO $$ DECLARE n int; BEGIN
  ...
  ELSE SELECT count(*) INTO n FROM public.disponibilidad_medico WHERE medico_id=NULLIF(current_setting('probe.fm_medgt',true), '')::uuid;
...
DO $$ BEGIN
  ...
  DELETE FROM public.planes_visitador_contratados WHERE empresa_id = NULLIF(current_setting('probe.pa_ea',true), '')::uuid;
  DELETE FROM public.planes_asignaciones WHERE empresa_id = NULLIF(current_setting('probe.pa_ea',true), '')::uuid;
  ...
  INSERT INTO public.planes_asignaciones (empresa_id,fecha_inicio,tipo_ciclo,precio_aplicado,moneda,estado,visitas_usadas)
    VALUES (NULLIF(current_setting('probe.pa_ea',true), '')::uuid, CURRENT_DATE, 'mensual', 0, 'GTQ', 'activo', 0);
...
DO $$ BEGIN IF current_setting('probe.va2_ready',true)='1' THEN
  INSERT INTO public.planes_visitador_contratados (empresa_id,plan_visitador_id,pais_id,cantidad_visitas_incluidas,visitas_usadas,precio_pagado,fecha_inicio,fecha_fin,estado)
    VALUES (NULLIF(current_setting('probe.pa_ea',true), '')::uuid, 1, NULLIF(current_setting('probe.p0_gt',true), '')::uuid, 10,0,100,CURRENT_DATE-1,CURRENT_DATE+30,'activo');
END IF; EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.b2_fallos', coalesce(current_setting('probe.b2_fallos', true),'')||'L4485('||SQLSTATE||') ', false); END $$;
...
SELECT set_config('role','none',true);
DO $$ BEGIN IF current_setting('probe.va2_ready',true)='1' THEN
  UPDATE public.planes_visitador_contratados SET fecha_fin=CURRENT_DATE-1 WHERE empresa_id=NULLIF(current_setting('probe.pa_ea',true), '')::uuid;
END IF; EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.b2_fallos', coalesce(current_setting('probe.b2_fallos', true),'')||'L4519('||SQLSTATE||') ', false); END $$;
SELECT set_config('request.jwt.claims', json_build_object('sub', current_setting('probe.cat_inv',true), 'role','authenticated')::text, true);
SELECT set_config('role','authenticated',true);
DO $$ BEGIN
...
END IF; EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.b2_fallos', coalesce(current_setting('probe.b2_fallos', true),'')||'L4537('||SQLSTATE||') ', false); END $$;
SELECT set_config('request.jwt.claims', json_build_object('sub', current_setting('probe.cat_inv',true), 'role','authenticated')::text, true);
SELECT set_config('role','authenticated',true);
DO $$ DECLARE v_id uuid; BEGIN
...
DO $$ DECLARE v_pac bigint; v_pacu uuid; BEGIN
  ...
  UPDATE public.pacientes SET pais_id = NULLIF(current_setting('probe.p0_gt',true), '')::uuid WHERE id = v_pac;   -- paciente GT
  -- slot contexto='paciente' para el médico HN (vg1_medhn) — la "carnada" cross-país
  INSERT INTO public.disponibilidad_medico (medico_id,dia_semana,hora_inicio,hora_fin,duracion_slot,activo,contexto)
    VALUES (NULLIF(current_setting('probe.vg1_medhn',true), '')::uuid, 3,'09:00','10:00',30,true,'paciente');
...
DO $$ DECLARE n int; BEGIN
  ...
  ELSE SELECT count(*) INTO n FROM public.disponibilidad_medico
       WHERE medico_id=NULLIF(current_setting('probe.vg1_medhn',true), '')::uuid AND contexto='paciente';
...
DO $$ DECLARE n int; BEGIN
  ...
  ELSE SELECT count(*) INTO n FROM public.disponibilidad_medico
       WHERE medico_id=NULLIF(current_setting('probe.fm_medgt',true), '')::uuid AND contexto='paciente';
...
DO $$ DECLARE n int; r boolean; BEGIN
  ...
  SELECT count(*) INTO n FROM public.recetas WHERE medico_id=NULLIF(current_setting('probe.acp_medA',true), '')::uuid;
  ...
  SELECT count(*) INTO n FROM public.receta_items WHERE receta_id=NULLIF(current_setting('probe.acp_recA',true), '')::bigint;
  ...
  SELECT count(*) INTO n FROM public.expediente_notas WHERE medico_id=NULLIF(current_setting('probe.acp_medA',true), '')::uuid;
  ...
  SELECT count(*) INTO n FROM public.signos_vitales WHERE medico_id=NULLIF(current_setting('probe.acp_medA',true), '')::uuid;
  ...
  SELECT count(*) INTO n FROM public.examenes WHERE medico_id=NULLIF(current_setting('probe.acp_medA',true), '')::uuid;
  ...
  SELECT count(*) INTO n FROM public.pacientes WHERE id=NULLIF(current_setting('probe.acp_pacA',true), '')::bigint;
...
DO $$ DECLARE n int; BEGIN
  ...
  ELSE SELECT count(*) INTO n FROM public.recetas WHERE paciente_id=NULLIF(current_setting('probe.acp_pacA',true), '')::bigint;  -- receta de PacA (ajena a paciente_u)
...
DO $$ DECLARE n int; BEGIN
  ...
  ELSE SELECT count(*) INTO n FROM public.citas WHERE clinica_id=NULLIF(current_setting('probe.acp_clinA',true), '')::uuid;
...
DO $$ DECLARE n int; BEGIN
  ...
  ELSE SELECT (SELECT count(*) FROM public.historial_medico WHERE medico_id=current_setting('probe.acp_medA',true))
            + (SELECT count(*) FROM public.recetas WHERE medico_id=NULLIF(current_setting('probe.acp_medA',true), '')::uuid)
            + (SELECT count(*) FROM public.pacientes WHERE id=NULLIF(current_setting('probe.acp_pacA',true), '')::bigint) INTO n;
...
DO $$ DECLARE n int; BEGIN
  ...
  ELSE SELECT count(*) INTO n FROM public.recetas WHERE medico_id=NULLIF(current_setting('probe.acp_medB',true), '')::uuid;
...
DO $$ DECLARE v_sol uuid; v_pais uuid; BEGIN
  ...
  SELECT pais_id INTO v_pais FROM public.empresas_proveedoras WHERE id=NULLIF(current_setting('probe.af_emp',true), '')::uuid;
  INSERT INTO public.solicitudes_campana (empresa_id, cuenta_proveedor_id, titulo, fecha_inicio, fecha_fin, estado, pais_id)
    VALUES (NULLIF(current_setting('probe.af_emp',true), '')::uuid, NULLIF(current_setting('probe.af_mkt',true), '')::uuid, 'Probe estado-check', now(), now()+interval '7 days', 'borrador', v_pais)
...
DO $$ DECLARE n int; BEGIN
  ...
  ELSE SELECT count(*) INTO n FROM public.invitaciones_visitador WHERE empresa_id=NULLIF(current_setting('probe.inv_empA',true), '')::uuid AND estado='pendiente';
...
DO $$ DECLARE n int; BEGIN
  ...
  ELSE SELECT count(*) INTO n FROM public.invitaciones_visitador WHERE empresa_id=NULLIF(current_setting('probe.inv_empB',true), '')::uuid;
...
DO $$ DECLARE n int; BEGIN
  ...
  ELSE SELECT count(*) INTO n FROM public.invitaciones_visitador WHERE empresa_id=NULLIF(current_setting('probe.inv_empA',true), '')::uuid;
...
DO $$ DECLARE v_post boolean; BEGIN
  ...
  DELETE FROM public.visitas_agendadas WHERE empresa_id=NULLIF(current_setting('probe.pa_ea',true), '')::uuid;
  DELETE FROM public.planes_visitador_contratados WHERE empresa_id=NULLIF(current_setting('probe.pa_ea',true), '')::uuid;
  DELETE FROM public.planes_asignaciones WHERE empresa_id=NULLIF(current_setting('probe.pa_ea',true), '')::uuid;
  INSERT INTO public.planes_asignaciones (empresa_id,fecha_inicio,tipo_ciclo,precio_aplicado,moneda,estado,visitas_usadas)
    VALUES (NULLIF(current_setting('probe.pa_ea',true), '')::uuid, CURRENT_DATE, 'mensual', 0, 'GTQ', 'activo', 0);
...
DO $$ BEGIN IF current_setting('probe.c3_ready',true)='1' THEN
  INSERT INTO public.planes_visitador_contratados (empresa_id,plan_visitador_id,pais_id,cantidad_visitas_incluidas,visitas_usadas,precio_pagado,fecha_inicio,fecha_fin,estado)
    VALUES (NULLIF(current_setting('probe.pa_ea',true), '')::uuid, 1, NULLIF(current_setting('probe.p0_gt',true), '')::uuid, 1, 0, 0, CURRENT_DATE-1, CURRENT_DATE+30, 'activo');
END IF; EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.b2_fallos', coalesce(current_setting('probe.b2_fallos', true),'')||'L5466('||SQLSTATE||') ', false); END $$;
...
DO $$ DECLARE v_pais uuid; BEGIN
  ...
  ELSE SELECT pais_id INTO v_pais FROM public.visitas_agendadas WHERE id=NULLIF(current_setting('probe.c3_v1',true), '')::uuid;
    PERFORM set_config('probe.p325', CASE WHEN v_pais=NULLIF(current_setting('probe.p0_gt',true), '')::uuid THEN 'OK (pais_id estampado = país del médico)' ELSE 'ROJO (pais_id no estampado: '||coalesce(v_pais::text,'NULL')||')' END, false);
  END IF;
END $$;
...
DO $$ BEGIN IF NULLIF(current_setting('probe.c3_v1',true),'')IS NOT NULL THEN
  UPDATE public.visitas_agendadas SET estado='cancelada' WHERE id=NULLIF(current_setting('probe.c3_v1',true), '')::uuid;
END IF; EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.b2_fallos', coalesce(current_setting('probe.b2_fallos', true),'')||'L5517('||SQLSTATE||') ', false); END $$;
SELECT set_config('request.jwt.claims', json_build_object('sub',current_setting('probe.cat_inv',true),'role','authenticated')::text,true);
SELECT set_config('role','authenticated',true);
DO $$ BEGIN
...
EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.b2_fallos', coalesce(current_setting('probe.b2_fallos', true),'')||'L5621('||SQLSTATE||') ', false);
END $$;
SELECT set_config('role','none', true);
...
DO $$ DECLARE v_in int; BEGIN
  ...
  SELECT count(*) INTO v_in FROM public.envios_push_cola WHERE solicitud_id=NULLIF(current_setting('probe.psh_sid',true), '')::uuid AND audiencia='paciente' AND paciente_id=NULLIF(current_setting('probe.psh_pca',true), '')::int;
...
DO $$ DECLARE v_in int; BEGIN
  ...
  SELECT count(*) INTO v_in FROM public.envios_push_cola WHERE solicitud_id=NULLIF(current_setting('probe.psh_sid',true), '')::uuid AND audiencia='paciente' AND paciente_id=NULLIF(current_setting('probe.psh_pcb',true), '')::int;
...
EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.b2_fallos', coalesce(current_setting('probe.b2_fallos', true),'')||'L5692('||SQLSTATE||') ', false);
END $$;
SELECT set_config('role','none', true);
DO $$ DECLARE v_in_cola int; v_in_vig int; BEGIN
  ...
  SELECT count(*) INTO v_in_cola FROM public.envios_push_cola WHERE solicitud_id=NULLIF(current_setting('probe.psh_sid',true), '')::uuid AND usuario_id=NULLIF(current_setting('probe.psh_a',true), '')::uuid AND estado='pendiente';
  SELECT count(*) INTO v_in_vig  FROM public.targets_push_vigentes(NULLIF(current_setting('probe.psh_sid',true), '')::uuid) WHERE usuario_id=NULLIF(current_setting('probe.psh_a',true), '')::uuid;
...
EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.b2_fallos', coalesce(current_setting('probe.b2_fallos', true),'')||'L5710('||SQLSTATE||') ', false);
END $$;
SELECT set_config('role','none', true);
DO $$ DECLARE v_in_cola int; v_in_vig int; BEGIN
  IF current_setting('probe.psh_ready',true)<>'1' OR coalesce(current_setting('probe.psh_sid',true),'')='' OR current_setting('probe.psh_pca',true)='' THEN
    PERFORM set_config('probe.p357','N/A',false); RETURN; END IF;
  SELECT count(*) INTO v_in_cola FROM public.envios_push_cola WHERE solicitud_id=NULLIF(current_setting('probe.psh_sid',true), '')::uuid AND paciente_id=NULLIF(current_setting('probe.psh_pca',true), '')::int AND estado='pendiente';
  SELECT count(*) INTO v_in_vig  FROM public.targets_push_vigentes(NULLIF(current_setting('probe.psh_sid',true), '')::uuid) WHERE paciente_id=NULLIF(current_setting('probe.psh_pca',true), '')::int;
  IF v_in_cola=1 AND v_in_vig=0
...
DO $$ DECLARE v_pend int; v_vig int; v_opt int; BEGIN
  ...
  SELECT count(*) INTO v_pend FROM public.envios_push_cola WHERE solicitud_id=NULLIF(current_setting('probe.psh_sid',true), '')::uuid AND estado='pendiente';
  SELECT count(*) INTO v_vig  FROM public.targets_push_vigentes(NULLIF(current_setting('probe.psh_sid',true), '')::uuid);
  SELECT count(*) INTO v_opt FROM public.envios_push_cola c
    ...
    WHERE c.solicitud_id=NULLIF(current_setting('probe.psh_sid',true), '')::uuid AND c.estado='pendiente'
...
DO $$ DECLARE v_first int; v_second int; BEGIN
  ...
  SELECT count(*) INTO v_first  FROM public.drenar_targets_push(NULLIF(current_setting('probe.psh_sid',true), '')::uuid);
  SELECT count(*) INTO v_second FROM public.drenar_targets_push(NULLIF(current_setting('probe.psh_sid',true), '')::uuid);
...
DO $$ DECLARE v_cam RECORD; v_gen boolean; v_nid text; BEGIN
  ...
  SELECT EXISTS(SELECT 1 FROM public.notificaciones
                WHERE usuario_id=NULLIF(current_setting('probe.ptx_pauth',true), '')::uuid
...
DO $$ DECLARE v_cam RECORD; v_gen boolean; v_nid text; BEGIN
  ...
  SELECT EXISTS(SELECT 1 FROM public.notificaciones
                WHERE usuario_id=NULLIF(current_setting('probe.ptx2_pauth',true), '')::uuid
...
DO $$ DECLARE v_lab_n int; v_lab_esp int; v_fuera int; v_pac_n int; v_pac_resumen boolean; v_e3 text; BEGIN
  ...
  SELECT count(*) INTO v_lab_n FROM public.notificaciones n JOIN public.cuentas_proveedor cp ON cp.id=n.usuario_id
    WHERE cp.empresa_id=NULLIF(current_setting('probe.lx_lab',true), '')::uuid AND n.tipo='orden_examen' AND n.titulo='Nueva orden de examen'
      AND n.accion_url='/laboratorio/ordenes' AND n.created_at > now() - interval '2 minutes';
  SELECT count(*) INTO v_lab_esp FROM public.cuentas_proveedor WHERE empresa_id=NULLIF(current_setting('probe.lx_lab',true), '')::uuid AND activo;
  SELECT count(*) INTO v_fuera FROM public.notificaciones n JOIN public.cuentas_proveedor cp ON cp.id=n.usuario_id
    WHERE n.tipo='orden_examen' AND n.titulo='Nueva orden de examen' AND n.created_at > now() - interval '2 minutes'
      AND cp.empresa_id IS DISTINCT FROM NULLIF(current_setting('probe.lx_lab',true), '')::uuid;
  -- PACIENTE: EXACTAMENTE 1 campanita por orden (NO N=lx_ocount), con el resumen 'N exámenes' en el mensaje
  SELECT count(*) INTO v_pac_n FROM public.notificaciones_pacientes WHERE paciente_id=NULLIF(current_setting('probe.lx_pac',true), '')::int
    AND tipo='examen' AND titulo='Nueva orden de examen' AND accion_url='/paciente/examenes' AND created_at > now() - interval '2 minutes';
  SELECT EXISTS(SELECT 1 FROM public.notificaciones_pacientes WHERE paciente_id=NULLIF(current_setting('probe.lx_pac',true), '')::int
...
DO $$ DECLARE v_err text; v_partial int; BEGIN
  ...
  SELECT (SELECT count(*) FROM public.notificaciones_pacientes WHERE paciente_id=NULLIF(current_setting('probe.lx_pac',true), '')::int AND titulo='Resultado de examen listo' AND created_at>now()-interval '2 minutes')
       + (SELECT count(*) FROM public.notificaciones WHERE usuario_id=NULLIF(current_setting('probe.lx_med',true), '')::uuid AND titulo='Resultado de examen listo' AND created_at>now()-interval '2 minutes') INTO v_partial;
...
DO $$ DECLARE v_pac_ok boolean; v_med_ok boolean; v_e4 text; BEGIN
  ...
  SELECT EXISTS(SELECT 1 FROM public.notificaciones_pacientes WHERE paciente_id=NULLIF(current_setting('probe.lx_pac',true), '')::int AND tipo='examen' AND titulo='Resultado de examen listo') INTO v_pac_ok;
  SELECT EXISTS(SELECT 1 FROM public.notificaciones WHERE usuario_id=NULLIF(current_setting('probe.lx_med',true), '')::uuid AND titulo='Resultado de examen listo' AND accion_url='/medico/citas') INTO v_med_ok;
...
DO $$ BEGIN IF current_setting('probe.lx_mut',true)<>'1' THEN PERFORM set_config('probe.p373','N/A',false); RETURN; END IF;
  BEGIN PERFORM public.notificar_orden_lab(NULLIF(current_setting('probe.lx_orden',true), '')::uuid); PERFORM set_config('probe.p373','PERMITIDO (multi-lab notificado — LEAK)',false);
  EXCEPTION WHEN others THEN PERFORM set_config('probe.p373','BLOQUEADO ('||SQLSTATE||')',false); END; END $$;
SELECT set_config('role','none', true);
DO $$ BEGIN IF current_setting('probe.lx_mut',true)='1' THEN UPDATE public.examenes SET laboratorio_id=NULLIF(current_setting('probe.lx_lab',true), '')::uuid WHERE id=NULLIF(current_setting('probe.lx_ex',true), '')::int; END IF; EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.b2_fallos', coalesce(current_setting('probe.b2_fallos', true),'')||'L6123('||SQLSTATE||') ', false); END $$;
...
DO $$ BEGIN IF current_setting('probe.lx_mut',true)<>'1' THEN PERFORM set_config('probe.p374','N/A',false); RETURN; END IF;
  BEGIN PERFORM public.notificar_orden_lab(NULLIF(current_setting('probe.lx_orden',true), '')::uuid); PERFORM set_config('probe.p374','PERMITIDO (multi-paciente notificado — LEAK PHI)',false);
  EXCEPTION WHEN others THEN PERFORM set_config('probe.p374','BLOQUEADO ('||SQLSTATE||')',false); END; END $$;
SELECT set_config('role','none', true);
DO $$ BEGIN IF current_setting('probe.lx_mut',true)='1' THEN UPDATE public.examenes SET paciente_id=NULLIF(current_setting('probe.lx_pac',true), '')::int WHERE id=NULLIF(current_setting('probe.lx_ex',true), '')::int; END IF; EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.b2_fallos', coalesce(current_setting('probe.b2_fallos', true),'')||'L6136('||SQLSTATE||') ', false); END $$;
...
SELECT set_config('role','none', true);
DO $$ BEGIN IF current_setting('probe.lx_mut',true)='1' THEN UPDATE public.examenes SET medico_id=NULLIF(current_setting('probe.lx_med',true), '')::uuid WHERE id=NULLIF(current_setting('probe.lx_ex',true), '')::int; END IF; EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.b2_fallos', coalesce(current_setting('probe.b2_fallos', true),'')||'L6149('||SQLSTATE||') ', false); END $$;
...
SELECT set_config('role','none', true);
DO $$ BEGIN IF current_setting('probe.lx_mut',true)='1' THEN UPDATE public.examenes SET paciente_id=NULLIF(current_setting('probe.lx_pac',true), '')::int WHERE id=NULLIF(current_setting('probe.lx_ex',true), '')::int; END IF; EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.b2_fallos', coalesce(current_setting('probe.b2_fallos', true),'')||'L6162('||SQLSTATE||') ', false); END $$;
...
-- ############################################################################################
-- PAQUETE PA-FAILOPEN - LOTE 5 - P507-P510 + FX10 (notificaciones, campanas y canjes, mig 270)
-- ############################################################################################
DO $$
DECLARE
  v_gt uuid := 'cbbbbe6d-59fe-4cf2-91ee-3e31ba1d5909';
  v_sv uuid := 'f2c75b8e-ef54-4a05-b2ff-7363e448f680';
  v_sinperfil uuid; v_super uuid; v_adminpais uuid;
  v_emp_gt uuid; v_emp_sv uuid; v_cta uuid; v_pac bigint;
  v_pago_gt uuid; v_pago_sv uuid;
  v_solpub_gt uuid; v_solpub_sv uuid;                    -- 'publicada' -> notificar_campana_resultado
  v_solpend_np uuid; v_solpend_sa uuid; v_solpend_ap uuid; v_solpend_sv uuid;  -- 'enviada' -> aprobar (el CHECK admite borrador|enviada|publicada|rechazada, NO 'pendiente')
  v_premio_gt bigint; v_premio_sv bigint;
  v_canje_np bigint; v_canje_sa bigint; v_canje_ap bigint; v_canje_sv bigint;
  v_notif0 bigint; v_notif1 bigint; v_camp0 bigint; v_camp1 bigint;
  v_est_sol text; v_est_canje text;
  v_det text := ''; v_ok boolean;
  v_507 text; v_508 text; v_509 text; v_510 text; v_fx text;
  v_fatal text := NULL; r record;
BEGIN
  IF to_regprocedure('private.puede_admin_pais(uuid,text[])') IS NULL THEN
    PERFORM set_config('probe.p507','ROJO (mig 265 sin aplicar)', false);
    PERFORM set_config('probe.p508','ROJO (mig 265 sin aplicar)', false);
    PERFORM set_config('probe.p509','ROJO (mig 265 sin aplicar)', false);
    PERFORM set_config('probe.p510','ROJO (mig 265 sin aplicar)', false);
    PERFORM set_config('probe.fx_l5','ROJO (mig 265 sin aplicar)', false);
    RETURN;
  END IF;
  ...
  BEGIN
    ...
    SELECT cp.empresa_id, cp.id INTO v_emp_gt, v_cta FROM public.cuentas_proveedor cp
      ...
      WHERE e.pais_id=v_gt AND cp.activo LIMIT 1;
    SELECT id INTO v_pac FROM public.pacientes WHERE auth_user_id IS NOT NULL LIMIT 1;
    IF v_sinperfil IS NULL OR v_super IS NULL OR v_adminpais IS NULL OR v_emp_gt IS NULL OR v_pac IS NULL THEN
      v_fatal := 'faltan actores base'; RAISE EXCEPTION 'PROBE_UNDO';
    END IF;

    INSERT INTO public.empresas_proveedoras (nombre_empresa, tipo, estado, pais_id, email_contacto)
      VALUES ('P270 Empresa SV','farmacia','activa', v_sv, 'p270.sv@example.invalid') RETURNING id INTO v_emp_sv;

    -- pagos en estado NOTIFICABLE ('verificado')
    INSERT INTO public.pagos_proveedor (empresa_id, tipo, monto, moneda, estado)
      VALUES (v_emp_gt,'plan',100,'GTQ','verificado') RETURNING id INTO v_pago_gt;
    INSERT INTO public.pagos_proveedor (empresa_id, tipo, monto, moneda, estado)
      VALUES (v_emp_sv,'plan',100,'USD','verificado') RETURNING id INTO v_pago_sv;

    -- solicitudes de campana: 'publicada' (notificable) y 'pendiente' (aprobable, una por actor)
    INSERT INTO public.solicitudes_campana (empresa_id, cuenta_proveedor_id, titulo, tipo,
                                            fecha_inicio, fecha_fin, pais_id, estado)
      VALUES (v_emp_gt, v_cta,'P270 Camp GT pub','banner', CURRENT_DATE, CURRENT_DATE+30, v_gt,'publicada')
      RETURNING id INTO v_solpub_gt;
    INSERT INTO public.solicitudes_campana (empresa_id, cuenta_proveedor_id, titulo, tipo,
                                            fecha_inicio, fecha_fin, pais_id, estado)
      VALUES (v_emp_sv, v_cta,'P270 Camp SV pub','banner', CURRENT_DATE, CURRENT_DATE+30, v_sv,'publicada')
      RETURNING id INTO v_solpub_sv;
    INSERT INTO public.solicitudes_campana (empresa_id, cuenta_proveedor_id, titulo, tipo,
                                            fecha_inicio, fecha_fin, pais_id, estado)
      VALUES (v_emp_gt, v_cta,'P270 Camp NP','banner', CURRENT_DATE, CURRENT_DATE+30, v_gt,'enviada')
      RETURNING id INTO v_solpend_np;
    INSERT INTO public.solicitudes_campana (empresa_id, cuenta_proveedor_id, titulo, tipo,
                                            fecha_inicio, fecha_fin, pais_id, estado)
      VALUES (v_emp_gt, v_cta,'P270 Camp SA','banner', CURRENT_DATE, CURRENT_DATE+30, v_gt,'enviada')
      RETURNING id INTO v_solpend_sa;
    INSERT INTO public.solicitudes_campana (empresa_id, cuenta_proveedor_id, titulo, tipo,
                                            fecha_inicio, fecha_fin, pais_id, estado)
      VALUES (v_emp_gt, v_cta,'P270 Camp AP','banner', CURRENT_DATE, CURRENT_DATE+30, v_gt,'enviada')
      RETURNING id INTO v_solpend_ap;
    INSERT INTO public.solicitudes_campana (empresa_id, cuenta_proveedor_id, titulo, tipo,
                                            fecha_inicio, fecha_fin, pais_id, estado)
      VALUES (v_emp_sv, v_cta,'P270 Camp SV pend','banner', CURRENT_DATE, CURRENT_DATE+30, v_sv,'enviada')
      RETURNING id INTO v_solpend_sv;

    -- canjes pendientes (el pais sale de premios.pais_id); resolver_canje CONSUME -> uno por actor
    INSERT INTO public.premios (nombre, costo_puntos, pais_id, tipo, activo)
      VALUES ('P270 Premio GT', 10, v_gt, 'fisico', true) RETURNING id INTO v_premio_gt;
    INSERT INTO public.premios (nombre, costo_puntos, pais_id, tipo, activo)
      VALUES ('P270 Premio SV', 10, v_sv, 'fisico', true) RETURNING id INTO v_premio_sv;
    INSERT INTO public.canjes (paciente_id, premio_id, costo_puntos, estado)
      VALUES (v_pac, v_premio_gt, 10, 'pendiente') RETURNING id INTO v_canje_np;
    INSERT INTO public.canjes (paciente_id, premio_id, costo_puntos, estado)
      VALUES (v_pac, v_premio_gt, 10, 'pendiente') RETURNING id INTO v_canje_sa;
    INSERT INTO public.canjes (paciente_id, premio_id, costo_puntos, estado)
      VALUES (v_pac, v_premio_gt, 10, 'pendiente') RETURNING id INTO v_canje_ap;
    INSERT INTO public.canjes (paciente_id, premio_id, costo_puntos, estado)
      VALUES (v_pac, v_premio_sv, 10, 'pendiente') RETURNING id INTO v_canje_sv;

    v_fx := 'OK (empresa SV, 2 pagos, 6 solicitudes de campana, 2 premios, 4 canjes)';

    -- ===================== P507: SIN PERFIL, MIDIENDO EFECTO =====================
    SELECT count(*) INTO v_notif0 FROM public.notificaciones;
    SELECT count(*) INTO v_camp0  FROM public.campanas_publicitarias;
    ...
    BEGIN PERFORM public.notificar_pago_resultado(v_pago_gt); v_det := v_det||'notif_pago=PASO! ';
    EXCEPTION WHEN OTHERS THEN
      v_det := v_det||'notif_pago='||CASE WHEN SQLSTATE='PT002' THEN 'BLOQ' ELSE 'otro('||SQLSTATE||')' END||' '; END;
    ...
    EXCEPTION WHEN OTHERS THEN
      v_det := v_det||'resolver_canje='||CASE WHEN SQLSTATE='P0001' AND SQLERRM='no_autorizado'
               THEN 'BLOQ' ELSE 'otro('||SQLSTATE||')' END||' '; END;

    PERFORM set_config('role','none', true);

    SELECT count(*) INTO v_notif1 FROM public.notificaciones;
    SELECT count(*) INTO v_camp1  FROM public.campanas_publicitarias;
    SELECT estado INTO v_est_sol   FROM public.solicitudes_campana WHERE id=v_solpend_np;
    SELECT estado INTO v_est_canje FROM public.canjes              WHERE id=v_canje_np;

    v_ok := v_det NOT LIKE '%PASO!%' AND v_det NOT LIKE '%otro(%'
        AND v_notif1=v_notif0 AND v_camp1=v_camp0
        AND v_est_sol='enviada' AND v_est_canje='pendiente';
    v_507 := CASE WHEN v_ok
      THEN 'BLOQUEADO en las 4 con la senal correcta, Y SIN EFECTO: notificaciones '||v_notif0||'='||v_notif1
           ||', campanas '||v_camp0||'='||v_camp1||', solicitud sigue '||v_est_sol||' (enviada), canje sigue '||v_est_canje
      ELSE 'ROJO ('||v_det||'| notificaciones '||v_notif0||'->'||v_notif1||' campanas '||v_camp0||'->'||v_camp1
           ||' solicitud='||COALESCE(v_est_sol,'?')||' canje='||COALESCE(v_est_canje,'?')||')' END;

    -- ===================== P508 / P509: NO REGRESION =====================
    FOR r IN SELECT * FROM (VALUES ('super', v_super, v_solpend_sa, v_canje_sa),
                                   ('adminpais', v_adminpais, v_solpend_ap, v_canje_ap)) t(etq,uid,sol,canje) LOOP
      PERFORM set_config('request.jwt.claims',
        json_build_object('sub', r.uid::text,'role','authenticated')::text, true);
      PERFORM set_config('role','authenticated', true);
      v_det := '';
    ...
    PERFORM set_config('role','authenticated', true);
    v_det := '';
    BEGIN PERFORM public.notificar_pago_resultado(v_pago_sv); v_det := v_det||'notif_pago=PASO! ';
    EXCEPTION WHEN OTHERS THEN v_det := v_det||'notif_pago='||CASE WHEN SQLSTATE='PT002' THEN 'BLOQ' ELSE 'otro('||SQLSTATE||')' END||' '; END;
    ...
    SELECT estado INTO v_est_sol   FROM public.solicitudes_campana WHERE id=v_solpend_sv;
    SELECT estado INTO v_est_canje FROM public.canjes              WHERE id=v_canje_sv;
    v_ok := v_det NOT LIKE '%PASO!%' AND v_det NOT LIKE '%otro(%'
        AND v_est_sol='enviada' AND v_est_canje='pendiente';
    v_510 := CASE WHEN v_ok
      THEN 'BLOQUEADO (admin de GT no notifica ni resuelve nada de SV; los registros de SV siguen '||v_est_sol||'/'||v_est_canje||')'
      ELSE 'ROJO — FUGA CROSS-PAIS ('||v_det||'| solicitud='||COALESCE(v_est_sol,'?')||' canje='||COALESCE(v_est_canje,'?')||')' END;
  ...
  PERFORM set_config('probe.fx_l5', COALESCE('ROJO (fixtures: '||v_fatal||')', v_fx, 'ROJO (sin resultado)'), false);
  PERFORM set_config('probe.p507',  COALESCE('ROJO (fatal: '||v_fatal||')', v_507, 'ROJO (sin resultado)'), false);
  PERFORM set_config('probe.p508',  COALESCE('ROJO (fatal: '||v_fatal||')', v_508, 'ROJO (sin resultado)'), false);
  PERFORM set_config('probe.p509',  COALESCE('ROJO (fatal: '||v_fatal||')', v_509, 'ROJO (sin resultado)'), false);
  PERFORM set_config('probe.p510',  COALESCE('ROJO (fatal: '||v_fatal||')', v_510, 'ROJO (sin resultado)'), false);
END $$;
SELECT set_config('role','none', true);
...
--                            bloque sin handler. La regla es ESTRICTA — un `RAISE EXCEPTION` NO cuenta
--                            como handler, es lo contrario; el detector ademas quita los literales SQL
--                            antes de buscar (b2_guard_test.py fija las dos direcciones del error). Cuando uno envuelto cae, su veredicto queda
...
-- ############################################################################################
-- MODULO COMERCIAL · FRENTE VISITAS, mig 273 (P535-P553). RED-FIRST.
-- --------------------------------------------------------------------------------------------
-- Mismo texto de tres estados que la 272:
--   'ROJO — AUSENTE'       la funcion/tabla no existe          <- rojo legitimo de la fase red
--   'ROJO (PERMITIO...)'   existe y NO rechazo                 <- el bug que se busca
--   'OK (...)'             rechazo con el errcode esperado, o el valor correcto
--
-- LA PROBE MAS IMPORTANTE DE ESTE LOTE ES P551: config de pais AUSENTE y el check-in a 5 km
-- sigue dando verificado=false. Un radio NULL que valide todo seria el mismo fail-open
-- trivaluado que costo siete migraciones en PA-FAILOPEN.
-- ############################################################################################
SELECT set_config('role','none', true);
DO $$
DECLARE v_gt uuid; v_ase1 uuid; v_ase2 uuid; v_sup uuid; v_admgt uuid; v_asehn uuid;
        v_pgeo uuid; v_pnogeo uuid; v_pajeno uuid; v_ok boolean;
        -- Ciudad de Guatemala. 1 grado de latitud ~ 111320 m.
        v_lat numeric := 14.6349; v_lng numeric := -90.5069;
BEGIN
  IF current_setting('probe.co_ready',true)<>'1' THEN PERFORM set_config('probe.vj_ready','0',false); RETURN; END IF;
  IF to_regclass('public.visitas_comerciales') IS NULL THEN
    PERFORM set_config('probe.vj_ready','0',false);
    PERFORM set_config('probe.vj_fx','ROJO — TABLAS AUSENTES (la mig 273 no esta aplicada; esto NO es un rechazo)',false);
    RETURN;
  END IF;
  ...
  v_asehn := NULLIF(current_setting('probe.co_asehn',true),'')::uuid;

  -- D5 fail-closed: NO se siembra config para GT. La ausencia es el estado inicial normal y es
  -- justo lo que P551 mide.
  DELETE FROM public.config_visitas_pais WHERE pais_id = v_gt;

  -- prospecto CON coordenada, prospecto SIN coordenada, prospecto de OTRO asesor
  INSERT INTO public.prospectos (nombre, tipo, pais_id, asesor_id, creado_por, estado_pipeline, lat, lng)
    VALUES ('QA VJ prospecto con geo','farmacia',v_gt,v_ase1,v_admgt,'nuevo',v_lat,v_lng) RETURNING id INTO v_pgeo;
  INSERT INTO public.prospectos (nombre, tipo, pais_id, asesor_id, creado_por, estado_pipeline)
    VALUES ('QA VJ prospecto sin geo','farmacia',v_gt,v_ase1,v_admgt,'nuevo') RETURNING id INTO v_pnogeo;
  INSERT INTO public.prospectos (nombre, tipo, pais_id, asesor_id, creado_por, estado_pipeline)
    VALUES ('QA VJ prospecto de ase2','farmacia',v_gt,v_ase2,v_admgt,'nuevo') RETURNING id INTO v_pajeno;

  -- la ficha de co_asehn se desactiva: es el sujeto de PA008 (comercial con ficha inactiva)
  UPDATE public.asesores_perfil SET activo = false WHERE id = v_asehn;

  PERFORM set_config('probe.vj_lat',   v_lat::text, false);
  PERFORM set_config('probe.vj_lng',   v_lng::text, false);
  PERFORM set_config('probe.vj_pgeo',  v_pgeo::text, false);
  PERFORM set_config('probe.vj_pnogeo',v_pnogeo::text, false);
  PERFORM set_config('probe.vj_pajeno',v_pajeno::text, false);
  PERFORM set_config('probe.vj_ready','1', false);
  PERFORM set_config('probe.vj_fx','OK (fixture visitas: 3 prospectos, config GT ausente a proposito, ficha HN inactivada)', false);
EXCEPTION WHEN OTHERS THEN
  PERFORM set_config('probe.vj_ready','0',false);
  PERFORM set_config('probe.vj_fx','ROJO ('||SQLSTATE||' '||SQLERRM||')',false);
END $$;

-- P535 — abrir_jornada por un MEDICO -> 42501
...
SELECT set_config('role','authenticated', true);
DO $$ BEGIN
  IF coalesce(current_setting('probe.vj_ready',true),'')<>'1' THEN PERFORM set_config('probe.p535','N/A (fixture visitas ausente)',false); RETURN; END IF;
  IF to_regprocedure('public.abrir_jornada(numeric,numeric,numeric)') IS NULL THEN
    PERFORM set_config('probe.p535','ROJO — RPC AUSENTE (todavia no existe; esto NO es un rechazo)',false); RETURN; END IF;
  BEGIN
    PERFORM public.abrir_jornada(NULL,NULL,NULL);
    PERFORM set_config('probe.p535','ROJO (PERMITIO — un medico abrio jornada comercial)',false);
  EXCEPTION WHEN insufficient_privilege THEN PERFORM set_config('probe.p535','OK (42501 no_autorizado)',false);
            WHEN others THEN PERFORM set_config('probe.p535','FALLO (esperaba 42501, vino '||SQLSTATE||': '||SQLERRM||')',false);
  END;
EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.p535','FALLO ('||SQLSTATE||' '||SQLERRM||')',false); END $$;
SELECT set_config('role','none', true);

-- P536 — abrir_jornada por un comercial con la ficha INACTIVA -> PA008
SELECT set_config('request.jwt.claims', json_build_object('sub', current_setting('probe.co_asehn',true), 'role','authenticated')::text, true);
SELECT set_config('role','authenticated', true);
DO $$ BEGIN
  IF coalesce(current_setting('probe.vj_ready',true),'')<>'1' THEN PERFORM set_config('probe.p536','N/A',false); RETURN; END IF;
  IF to_regprocedure('public.abrir_jornada(numeric,numeric,numeric)') IS NULL THEN
    PERFORM set_config('probe.p536','ROJO — RPC AUSENTE',false); RETURN; END IF;
  BEGIN
    PERFORM public.abrir_jornada(NULL,NULL,NULL);
    PERFORM set_config('probe.p536','ROJO (PERMITIO — ficha inactiva abrio jornada)',false);
  EXCEPTION WHEN sqlstate 'PA008' THEN PERFORM set_config('probe.p536','OK (PA008)',false);
            WHEN others THEN PERFORM set_config('probe.p536','FALLO (esperaba PA008, vino '||SQLSTATE||': '||SQLERRM||')',false);
  END;
...
SELECT set_config('request.jwt.claims', json_build_object('sub', current_setting('probe.co_ase1',true), 'role','authenticated')::text, true);
SELECT set_config('role','authenticated', true);
DO $$ DECLARE v_j uuid; BEGIN
  IF coalesce(current_setting('probe.vj_ready',true),'')<>'1' THEN PERFORM set_config('probe.p537','N/A',false); RETURN; END IF;
  IF to_regprocedure('public.abrir_jornada(numeric,numeric,numeric)') IS NULL THEN
    PERFORM set_config('probe.p537','ROJO — RPC AUSENTE',false); RETURN; END IF;
  BEGIN
    SELECT public.abrir_jornada(NULL,NULL,NULL) INTO v_j;
    PERFORM set_config('probe.vj_jornada', coalesce(v_j::text,''), false);
  EXCEPTION WHEN others THEN
    PERFORM set_config('probe.p537','FALLO (la PRIMERA jornada no abrio: '||SQLSTATE||' '||SQLERRM||')',false); RETURN;
  END;
  BEGIN
    PERFORM public.abrir_jornada(NULL,NULL,NULL);
    PERFORM set_config('probe.p537','ROJO (PERMITIO dos jornadas el mismo dia)',false);
  EXCEPTION WHEN unique_violation THEN PERFORM set_config('probe.p537','OK (23505 UNIQUE por dia)',false);
            WHEN sqlstate 'PA020' THEN PERFORM set_config('probe.p537','OK (PA020 jornada ya abierta)',false);
            WHEN others THEN PERFORM set_config('probe.p537','FALLO (esperaba 23505/PA020, vino '||SQLSTATE||': '||SQLERRM||')',false);
  END;
EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.p537','FALLO ('||SQLSTATE||' '||SQLERRM||')',false); END $$;
SELECT set_config('role','none', true);

-- P538 — planificar_visita sobre un prospecto FUERA de la cartera -> 42501
SELECT set_config('request.jwt.claims', json_build_object('sub', current_setting('probe.co_ase1',true), 'role','authenticated')::text, true);
SELECT set_config('role','authenticated', true);
DO $$ BEGIN
  ...
  BEGIN
    ...
    PERFORM set_config('probe.p538','ROJO (PERMITIO planificar sobre prospecto ajeno)',false);
  EXCEPTION WHEN insufficient_privilege THEN PERFORM set_config('probe.p538','OK (42501 no_autorizado)',false);
            WHEN others THEN PERFORM set_config('probe.p538','FALLO (esperaba 42501, vino '||SQLSTATE||': '||SQLERRM||')',false);
  END;
EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.p538','FALLO ('||SQLSTATE||' '||SQLERRM||')',false); END $$;
SELECT set_config('role','none', true);

-- P539 — el SUPERVISOR planifica sobre un prospecto de SU asesor -> PERMITIDO
SELECT set_config('request.jwt.claims', json_build_object('sub', current_setting('probe.co_sup',true), 'role','authenticated')::text, true);
SELECT set_config('role','authenticated', true);
DO $$ DECLARE v_v uuid; BEGIN
  ...
  BEGIN
    ...
    PERFORM set_config('probe.vj_v_sup', coalesce(v_v::text,''), false);
    PERFORM set_config('probe.p539','OK (PERMITIDO: el supervisor planifica sobre la cartera de su asesor)',false);
  EXCEPTION WHEN others THEN
    PERFORM set_config('probe.p539','FALLO (esperaba PERMITIDO, vino '||SQLSTATE||': '||SQLERRM||')',false);
  END;
EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.p539','FALLO ('||SQLSTATE||' '||SQLERRM||')',false); END $$;
SELECT set_config('role','none', true);

-- P540 — VERIFICA LA FILA, no que "no exploto": el asesor_id tiene que ser el del ASESOR
DO $$ DECLARE v_ase uuid; BEGIN
  IF coalesce(current_setting('probe.vj_ready',true),'')<>'1' THEN PERFORM set_config('probe.p540','N/A',false); RETURN; END IF;
  IF coalesce(current_setting('probe.vj_v_sup',true),'')='' THEN
    PERFORM set_config('probe.p540','N/A (no se creo la visita en P539)',false); RETURN; END IF;
  SELECT asesor_id INTO v_ase FROM public.visitas_comerciales WHERE id = NULLIF(current_setting('probe.vj_v_sup',true),'')::uuid;
  IF v_ase = NULLIF(current_setting('probe.co_ase1',true),'')::uuid THEN
    PERFORM set_config('probe.p540','OK (la fila quedo con asesor_id del ASESOR, no del supervisor)',false);
  ELSIF v_ase = NULLIF(current_setting('probe.co_sup',true),'')::uuid THEN
    PERFORM set_config('probe.p540','ROJO (la visita quedo a nombre del SUPERVISOR — se apropio de la visita)',false);
  ELSE
    PERFORM set_config('probe.p540','FALLO (asesor_id inesperado: '||coalesce(v_ase::text,'nulo')||')',false);
  END IF;
EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.p540','FALLO ('||SQLSTATE||' '||SQLERRM||')',false); END $$;

-- Fixture de visitas para los check-ins.
-- UN PROSPECTO POR VISITA: la tabla tiene UNIQUE (prospecto_id, asesor_id, fecha_planificada), o
-- sea que nueve visitas del mismo asesor al mismo prospecto el mismo dia chocan entre si. La
-- primera version de este fixture hacia justo eso y las 12 probes de check-in salieron N/A.
DO $$
DECLARE v_gt uuid := NULLIF(current_setting('probe.co_gt',true),'')::uuid;
        ...
        v_pajeno uuid := NULLIF(current_setting('probe.vj_pajeno',true),'')::uuid;
        v_j uuid := NULLIF(current_setting('probe.vj_jornada',true),'')::uuid;
        v_lat numeric := NULLIF(current_setting('probe.vj_lat',true),'')::numeric;
        v_lng numeric := NULLIF(current_setting('probe.vj_lng',true),'')::numeric;
        r record; v_p uuid; v_id uuid;
BEGIN
  IF coalesce(current_setting('probe.vj_ready',true),'')<>'1' THEN RETURN; END IF;
  FOR r IN SELECT * FROM (VALUES
      ('vj_v_doble','geo'), ('vj_v_nogeo','nogeo'), ('vj_v_lejos','geo'), ('vj_v_prec','geo'),
      ('vj_v_dif','geo'), ('vj_v_cli1','geo'), ('vj_v_cli2','geo'), ('vj_v_cli3','geo'),
      ('vj_v_cerca','geo')
    ) AS t(clave, cual) LOOP
    INSERT INTO public.prospectos (nombre, tipo, pais_id, asesor_id, creado_por, estado_pipeline, lat, lng)
...
-- P541 — check-in hecho por el SUPERVISOR sobre la visita de su asesor -> 42501
-- El supervisor gestiona la cartera, pero un check-in ajeno no es un dato: es una falsificacion.
SELECT set_config('request.jwt.claims', json_build_object('sub', current_setting('probe.co_sup',true), 'role','authenticated')::text, true);
SELECT set_config('role','authenticated', true);
DO $$ BEGIN
  IF coalesce(current_setting('probe.vj_visitas',true),'')<>'1' THEN PERFORM set_config('probe.p541','N/A',false); RETURN; END IF;
  IF to_regprocedure('public.checkin_visita_comercial(uuid,numeric,numeric,numeric,timestamp with time zone)') IS NULL THEN
    PERFORM set_config('probe.p541','ROJO — RPC AUSENTE',false); RETURN; END IF;
  BEGIN
    PERFORM public.checkin_visita_comercial(NULLIF(current_setting('probe.vj_v_cerca',true),'')::uuid, 14.6349, -90.5069, 15, NULL);
    PERFORM set_config('probe.p541','ROJO (PERMITIO — el supervisor hizo el check-in de otro)',false);
  EXCEPTION WHEN insufficient_privilege THEN PERFORM set_config('probe.p541','OK (42501 no_autorizado)',false);
            WHEN others THEN PERFORM set_config('probe.p541','FALLO (esperaba 42501, vino '||SQLSTATE||': '||SQLERRM||')',false);
  END;
EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.p541','FALLO ('||SQLSTATE||' '||SQLERRM||')',false); END $$;
SELECT set_config('role','none', true);

-- P542 — check-in SIN jornada abierta (asesor2 nunca abrio) -> PA022
SELECT set_config('request.jwt.claims', json_build_object('sub', current_setting('probe.co_ase2',true), 'role','authenticated')::text, true);
SELECT set_config('role','authenticated', true);
DO $$ BEGIN
  IF coalesce(current_setting('probe.vj_visitas',true),'')<>'1' THEN PERFORM set_config('probe.p542','N/A',false); RETURN; END IF;
  IF to_regprocedure('public.checkin_visita_comercial(uuid,numeric,numeric,numeric,timestamp with time zone)') IS NULL THEN
    PERFORM set_config('probe.p542','ROJO — RPC AUSENTE',false); RETURN; END IF;
  BEGIN
    PERFORM public.checkin_visita_comercial(NULLIF(current_setting('probe.vj_v_ase2',true),'')::uuid, 14.6349, -90.5069, 15, NULL);
    PERFORM set_config('probe.p542','ROJO (PERMITIO check-in sin jornada abierta)',false);
  EXCEPTION WHEN sqlstate 'PA022' THEN PERFORM set_config('probe.p542','OK (PA022 sin jornada abierta)',false);
            WHEN others THEN PERFORM set_config('probe.p542','FALLO (esperaba PA022, vino '||SQLSTATE||': '||SQLERRM||')',false);
  END;
EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.p542','FALLO ('||SQLSTATE||' '||SQLERRM||')',false); END $$;
SELECT set_config('role','none', true);

-- ============================================================================================
-- Los check-ins del asesor1. Cada uno consume su propia visita.
-- ============================================================================================
SELECT set_config('request.jwt.claims', json_build_object('sub', current_setting('probe.co_ase1',true), 'role','authenticated')::text, true);
SELECT set_config('role','authenticated', true);

-- P543 — doble check-in sobre la misma visita
DO $$ BEGIN
  IF coalesce(current_setting('probe.vj_visitas',true),'')<>'1' THEN PERFORM set_config('probe.p543','N/A',false); RETURN; END IF;
  IF to_regprocedure('public.checkin_visita_comercial(uuid,numeric,numeric,numeric,timestamp with time zone)') IS NULL THEN
    PERFORM set_config('probe.p543','ROJO — RPC AUSENTE',false); RETURN; END IF;
  BEGIN
    PERFORM public.checkin_visita_comercial(NULLIF(current_setting('probe.vj_v_doble',true),'')::uuid, 14.6349, -90.5069, 15, NULL);
  EXCEPTION WHEN others THEN
    PERFORM set_config('probe.p543','FALLO (el PRIMER check-in no paso: '||SQLSTATE||' '||SQLERRM||')',false); RETURN;
  END;
  BEGIN
    PERFORM public.checkin_visita_comercial(NULLIF(current_setting('probe.vj_v_doble',true),'')::uuid, 14.6349, -90.5069, 15, NULL);
    PERFORM set_config('probe.p543','ROJO (PERMITIO un segundo check-in sobre la misma visita)',false);
  EXCEPTION WHEN sqlstate 'PA025' THEN PERFORM set_config('probe.p543','OK (PA025 doble check-in rechazado)',false);
            WHEN others THEN PERFORM set_config('probe.p543','FALLO (esperaba PA025, vino '||SQLSTATE||': '||SQLERRM||')',false);
  END;
EXCEPTION WHEN OTHERS THEN PERFORM set_config('probe.p543','FALLO ('||SQLSTATE||' '||SQLERRM||')',false); END $$;

-- P544/P545/P546/P547/P553 — se hacen los check-ins y DESPUES se leen las filas como owner.
DO $$ BEGIN
  IF coalesce(current_setting('probe.vj_visitas',true),'')<>'1' THEN RETURN; END IF;
  IF to_regprocedure('public.checkin_visita_comercial(uuid,numeric,numeric,numeric,timestamp with time zone)') IS NULL THEN RETURN; END IF;
  -- prospecto SIN coordenada
  BEGIN PERFORM public.checkin_visita_comercial(NULLIF(current_setting('probe.vj_v_nogeo',true),'')::uuid, 14.6349, -90.5069, 15, NULL);
  EXCEPTION WHEN others THEN PERFORM set_config('probe.vj_e_nogeo', SQLSTATE||' '||SQLERRM, false); END;
  -- a ~5 km (0.04492 grados de latitud)
  BEGIN PERFORM public.checkin_visita_comercial(NULLIF(current_setting('probe.vj_v_lejos',true),'')::uuid, 14.67982, -90.5069, 15, NULL);
  EXCEPTION WHEN others THEN PERFORM set_config('probe.vj_e_lejos', SQLSTATE||' '||SQLERRM, false); END;
  -- precision malisima
  ...
  BEGIN PERFORM public.checkin_visita_comercial(NULLIF(current_setting('probe.vj_v_cerca',true),'')::uuid, 14.63508, -90.5069, 15, NULL);
  EXCEPTION WHEN others THEN PERFORM set_config('probe.vj_e_cerca', SQLSTATE||' '||SQLERRM, false); END;
END $$;

-- P548/P549/P550 — cliente_at inconsistente -> PA024 (otro dia / futuro / anterior a la jornada)
DO $$ BEGIN
  ...
  BEGIN PERFORM public.checkin_visita_comercial(NULLIF(current_setting('probe.vj_v_cli1',true),'')::uuid, 14.6349, -90.5069, 15, now() - interval '2 days');
        PERFORM set_config('probe.p548','ROJO (PERMITIO un cliente_at de OTRO DIA)',false);
  EXCEPTION WHEN sqlstate 'PA024' THEN PERFORM set_config('probe.p548','OK (PA024 otro dia)',false);
            WHEN others THEN PERFORM set_config('probe.p548','FALLO (esperaba PA024, vino '||SQLSTATE||': '||SQLERRM||')',false); END;
  BEGIN PERFORM public.checkin_visita_comercial(NULLIF(current_setting('probe.vj_v_cli2',true),'')::uuid, 14.6349, -90.5069, 15, now() + interval '3 hours');
...
-- ============================================================================================
-- Lectura de las filas como owner: lo que quedo ESCRITO, no lo que devolvio la llamada.
-- ============================================================================================
DO $$
DECLARE r record; v_dist numeric;
BEGIN
  IF coalesce(current_setting('probe.vj_visitas',true),'')<>'1' OR to_regclass('public.visitas_comerciales') IS NULL THEN
    PERFORM set_config('probe.p544','N/A',false); PERFORM set_config('probe.p545','N/A',false);
    PERFORM set_config('probe.p546','N/A',false); PERFORM set_config('probe.p547','N/A',false);
    PERFORM set_config('probe.p551','N/A',false); PERFORM set_config('probe.p553','N/A',false); RETURN; END IF;

  -- P544 prospecto SIN coordenada: se registra igual, pero verificado=false CON motivo
  SELECT * INTO r FROM public.visitas_comerciales WHERE id = NULLIF(current_setting('probe.vj_v_nogeo',true),'')::uuid;
  PERFORM set_config('probe.p544', CASE
    WHEN r.checkin_at IS NULL THEN 'FALLO (no se registro el check-in: '||coalesce(current_setting('probe.vj_e_nogeo',true),'sin error')||')'
    WHEN COALESCE(r.checkin_verificado,true) THEN 'ROJO (verificado=true sin coordenada del prospecto — fail-open)'
    WHEN coalesce(r.checkin_motivo,'')='' THEN 'FALLO (verificado=false pero SIN motivo)'
    ELSE 'OK (registrado, verificado=false, motivo='||r.checkin_motivo||')' END, false);

  -- P545 a 5 km: verificado=false y la distancia la calculo la RPC (no llego por parametro)
  SELECT * INTO r FROM public.visitas_comerciales WHERE id = NULLIF(current_setting('probe.vj_v_lejos',true),'')::uuid;
  PERFORM set_config('probe.p545', CASE
    WHEN r.checkin_at IS NULL THEN 'FALLO (no se registro: '||coalesce(current_setting('probe.vj_e_lejos',true),'sin error')||')'
    WHEN COALESCE(r.checkin_verificado,true) THEN 'ROJO (verificado=true a 5 km)'
    WHEN r.checkin_distancia_m IS NULL THEN 'FALLO (verificado=false pero la RPC no calculo la distancia)'
    WHEN r.checkin_distancia_m < 4000 OR r.checkin_distancia_m > 6000 THEN 'FALLO (distancia calculada fuera de rango: '||round(r.checkin_distancia_m)||' m, se esperaban ~5000)'
    ELSE 'OK (verificado=false, distancia calculada por la RPC = '||round(r.checkin_distancia_m)||' m)' END, false);

  -- P551 — LA IMPORTANTE. Sin fila de config para GT, el default fail-closed igual rechaza.
  SELECT * INTO r FROM public.visitas_comerciales WHERE id = NULLIF(current_setting('probe.vj_v_lejos',true),'')::uuid;
  PERFORM set_config('probe.p551', CASE
    WHEN EXISTS (SELECT 1 FROM public.config_visitas_pais WHERE pais_id = NULLIF(current_setting('probe.co_gt',true),'')::uuid)
      THEN 'FALLO (la probe no mide nada: HAY fila de config para GT)'
    WHEN r.checkin_at IS NULL THEN 'FALLO (no se registro el check-in)'
    WHEN COALESCE(r.checkin_verificado,true)
      THEN 'ROJO (SIN config de pais el radio quedo abierto y valido un check-in a 5 km — fail-OPEN)'
    ELSE 'OK (config de GT AUSENTE y el default fail-closed igual rechazo los 5 km)' END, false);

  -- P546 precision malisima
  SELECT * INTO r FROM public.visitas_comerciales WHERE id = NULLIF(current_setting('probe.vj_v_prec',true),'')::uuid;
  PERFORM set_config('probe.p546', CASE
    WHEN r.checkin_at IS NULL THEN 'FALLO (no se registro: '||coalesce(current_setting('probe.vj_e_prec',true),'sin error')||')'
  ...
  PERFORM set_config('probe.p547', CASE
    ...
    WHEN COALESCE(r.checkin_verificado,true) THEN 'ROJO (un check-in DIFERIDO quedo verificado=true)'
  ...
  PERFORM set_config('probe.p553', CASE
    WHEN r.checkin_at IS NULL THEN 'FALLO (no se registro: '||coalesce(current_setting('probe.vj_e_cerca',true),'sin error')||')'
    WHEN NOT COALESCE(r.checkin_verificado,false)
      THEN 'ROJO (a 20 m con precision 15 NO verifico: el booleano no puede ser true nunca, no mide nada. distancia='||coalesce(round(r.checkin_distancia_m)::text,'nula')||' motivo='||coalesce(r.checkin_motivo,'-')||')'
    ELSE 'OK (verificado=true a '||coalesce(round(r.checkin_distancia_m)::text,'?')||' m — el booleano SI puede ser true)' END, false);
EXCEPTION WHEN OTHERS THEN
...
UNION ALL SELECT 'VJ_FX_fixture_visitas',             current_setting('probe.vj_fx', true),          'OK'
UNION ALL SELECT 'VJ_FX2_fixture_visitas_filas',      current_setting('probe.vj_fx2', true),         'OK'
UNION ALL SELECT 'P535_vj_jornada_por_medico',           current_setting('probe.p535', true),          'OK (42501 no_autorizado)'
UNION ALL SELECT 'P536_vj_jornada_ficha_inactiva',       current_setting('probe.p536', true),          'OK (PA008)'
UNION ALL SELECT 'P537_vj_dos_jornadas_mismo_dia',       current_setting('probe.p537', true),          'OK (23505 o PA020)'
UNION ALL SELECT 'P538_vj_planificar_fuera_de_cartera',  current_setting('probe.p538', true),          'OK (42501 no_autorizado)'
UNION ALL SELECT 'P539_vj_supervisor_planifica_permitido',  current_setting('probe.p539', true),          'OK (PERMITIDO)'
UNION ALL SELECT 'P540_vj_la_fila_queda_del_ASESOR',     current_setting('probe.p540', true),          'OK (asesor_id del asesor)'
UNION ALL SELECT 'P541_vj_checkin_por_el_supervisor',    current_setting('probe.p541', true),          'OK (42501 no_autorizado)'
UNION ALL SELECT 'P542_vj_checkin_sin_jornada',          current_setting('probe.p542', true),          'OK (PA022)'
UNION ALL SELECT 'P543_vj_doble_checkin',                current_setting('probe.p543', true),          'OK (PA025)'
UNION ALL SELECT 'P544_vj_checkin_prospecto_sin_geo',    current_setting('probe.p544', true),          'OK (verificado=false con motivo)'
UNION ALL SELECT 'P545_vj_checkin_a_5km',                current_setting('probe.p545', true),          'OK (verificado=false, distancia calculada)'
UNION ALL SELECT 'P546_vj_checkin_precision_mala',       current_setting('probe.p546', true),          'OK (verificado=false)'
UNION ALL SELECT 'P547_vj_checkin_diferido',             current_setting('probe.p547', true),          'OK (verificado=false SIEMPRE, origen=diferido)'
UNION ALL SELECT 'P548_vj_cliente_at_otro_dia',          current_setting('probe.p548', true),          'OK (PA024)'
UNION ALL SELECT 'P549_vj_cliente_at_futuro',            current_setting('probe.p549', true),          'OK (PA024)'
UNION ALL SELECT 'P550_vj_cliente_at_previo_jornada',    current_setting('probe.p550', true),          'OK (PA024)'
